...
def agregar_hotel(hoteles, clientes, reservas) -> None:
    ...
    nombre_hotel = nombre_hotel.lower().title() # Formateamos el nombre del hotel para que la primera letra de cada palabra sea mayúscula
    ubicacion = input("Ingrese la ciudad donde se encuentra el hotel: ") # Pedimos la ubicación del hotel
    ...
    while True:
        try:
            ...
            num_habitaciones_a_agregar = int(input("¿Cuántas habitaciones desea agregar a este hotel?: "))
            
            if num_habitaciones_a_agregar < 0: # Si la cantidad es negativa, mostramos un error
                print("Por favor, ingrese un número positivo.")
                continue
            break
        except ValueError: # Si el usuario ingresa algo que no sea un número, mostramos un error
            print("Error: Ingrese un número válido.")

    for i in range(num_habitaciones_a_agregar): # Recorremos la cantidad de habitaciones a agregar
        print(f"\n--- Agregando Habitación {i+1} ---")
        
        # Pedimos el nuumero de la habitación
        while True:
            try:
                numero = int(input(f"Número de la habitación {i+1}: ")) # Pedimos el número de la habitación

                # Verificamos que no exista otra habitación con el mismo número en el hotel
                if any(h['numero'] == numero for h in lista_habitaciones): # Si existe una habitación con el mismo número, mostramos un error
                   print(f"Error: Ya existe una habitación con el número {numero} en este hotel.")
                   continue
                break
            except ValueError:
                print("Error: Ingrese un número válido.")
        ...
        while True:
            try:
                capacidad = int(input(f"Capacidad de la habitación {numero}: ")) # Se pide la capacidad de la habitación
                if capacidad <= 0: # Si la capacidad es menor o igual a cero, mostramos un error
        ...
        while True:
            try:
                precio = float(input(f"Precio por noche de la habitación {numero}: ")) # Se pide el precio por noche de la habitación
                if precio <= 0: # Si el precio es menor o igual a cero, mostramos un error
    ...
    if hoteles:
        id_hotel = max(h['id'] for h in hoteles) + 1 # El ID va a ser el máximo ID actual + 1
    else:
...
def consultar_hoteles(hoteles: list) -> None:
    ...
    if hoteles: # Si hay hoteles registrados
        for hotel in hoteles: # Recorremos la lista de hoteles
            print(f"\nID: {hotel['id']} | Nombre: {hotel['nombre']} | Ubicación: {hotel['ubicacion']}") # Mostramos la información básica del hotel
            print("  Habitaciones:")
            if hotel['habitaciones']: # Si el hotel tiene habitaciones registradas
                for hab in hotel['habitaciones']: # Mostramos la información de cada habitación
                    print(f"    - Número: {hab['numero']}, Capacidad: {hab['capacidad']}, Precio: ${hab['precio']:.2f}")
            else:
...
def eliminar_hotel(hoteles: list, clientes: list, reservas: list) -> None:
    ...
    entrada = input("Ingrese el ID o el nombre del hotel que desea eliminar: ") # Pedimos el ID o nombre del hotel a eliminar, se elimina espacios en blanco al inicio y final
    ...
    try:
        ...
        for hotel in hoteles: # Recorremos la lista de hoteles
            if hotel['id'] == id_a_eliminar: # Si encontramos el hotel con el ID ingresado
        ...
        nombre_buscar = entrada.lower() # Convertimos la entrada a minusculas y la guardamos
        coincidencias = [h for h in hoteles if h['nombre'].lower() == nombre_buscar] # Buscamos por coincidencias en la lista hoteles y las almacenamos
        ...
        if len(coincidencias) == 0: # Si no hay coincidencias. Informamos al usuario
            ...
            print(f"Se encontraron {len(coincidencias)} hoteles con el nombre '{entrada}':") # Informamos la cantidad de hoteles encontrados
            for h in coincidencias:
                print(f"  ID: {h['id']} | Nombre: {h['nombre']} | Ubicación: {h.get('ubicacion', 'N/A')}") # Mostramos la informacion de los hoteles encontrados
            
            while True:
                try:
                    # Solicitamos al usuario que ingrese el ID del hotel que desea eliminar
                    id_elegido = int(input("Ingrese el ID del hotel que desea eliminar de las opciones anteriores: "))
    ...
    confirmacion = input(f"¿Está seguro que desea eliminar el hotel '{hotel_encontrado['nombre']}' (ID: {hotel_encontrado['id']})? (s/n): ")
    if confirmacion.lower() == 's': # Si el usuario confirma
        hoteles.remove(hotel_encontrado) # Eliminamos el hotel de la lista
        datos.guardar_datos(hoteles, clientes, reservas)  # Guardamos los cambios en el archivo
...
def gestionar_hoteles(hoteles: list, clientes: list, reservas: list) -> None:
    ...
    while True:
        ...
        if opcion == '1':
            limpiar_pantalla()
            # Pasamos las listas para que agregar_hotel pueda guardarlas
